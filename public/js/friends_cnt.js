        ...
        getUserById("", function(currUser) {
            friends.forEach(function(friend) {
                ...
                checkFriends(friend.id, function(result) {
                    ...
                    //  Else if friend is not current user, add "add friend" button.
                    else if (friend.id !== currUser.id)
                    ...
                    friendListElement.append("<div class='row member-row'>" +
                                                ...
                                                "</div>" + 
                                                "<div class='col-md-3'>" +
                                                    "<p><a id='friend-link_" + friend.id + "'href ='http://" + window.location.host + "/profile?id=" + friend.id + "' class='btn btn-primary btn-block'><i class='fa fa-edit'></i> View Profile</a></p>" + 
                                                "</div>" +
                                            "</div>");
                    
                    if (friend.id === currUser.id)
                        $("#friend-link_" + currUser.id).attr("href", "http://" + window.location.host + "/profile");
